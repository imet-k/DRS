...
gene_file = 'vp_transcripts_coding_new.txt'
#input file is a bed file generated from sorted bam alignment file using bedtools bamtobed
input_file = '../Mapping/n101.bed'
output_file_aware = 'n101.tss_intergenic_testing_wendsTEST.tab'
...
with open(gene_file, 'r') as gen, open(input_file, 'r') as bed, open(output_file_aware, 'w') as new:
    all_reads_neg = {'NC_004603.1': [], 'NC_004605.1': []}  # both genes and reads in their chromosome
    all_reads_pos = {'NC_004603.1': [], 'NC_004605.1': []}
    genes = {'NC_004603.1': [], 'NC_004605.1': []} #only genes
    for line in gen: #store genes coordinates first
        read = line.strip().split()
        if read[6] == '-':
            all_reads_neg[read[0]].append((int(read[3]), int(read[4]), read[8][3:], 0)) #zero in the end indicates that it is a gene not a read
        else:
            all_reads_pos[read[0]].append((int(read[3]), int(read[4]), read[8][3:], 0))
